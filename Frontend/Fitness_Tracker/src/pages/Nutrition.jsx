...
// Foods are now fetched from backend
...
const Nutrition = () => {
    ...
    const fetchData = async () => {
        if (!user) return;
        try {
            const [mealsRes, statsRes, foodsRes] = await Promise.all([
                axios.get(`${API_BASE_URL}/api/meals/${today}`, authConfig),
                axios.get(`${API_BASE_URL}/api/stats/${today}`, authConfig),
                axios.get(`${API_BASE_URL}/api/foods`, authConfig)
            ]);
            setMeals(mealsRes.data);
            setFoodDatabase(foodsRes.data);
            setStats({
            ...
            generateAIRecommendation(statsRes.data, foodsRes.data);
        } catch (err) {
    ...
    const generateAIRecommendation = (currentStats, foodsDb = foodDatabase) => {
        ...
        let target;
        if (remainingPro > 30) {
            target = foodsDb.find(f => f.pro > 20 && f.carb < 30);
        } else if (remainingCarb > 50) {
            target = foodsDb.find(f => f.carb > 40);
        } else {
            target = foodsDb.find(f => f.cal < 250);
        }
        setRecommendation(target || foodsDb[0]);
    };
        ...
        <main className="main nutrition-page">
            ...
            <div className="diet-log-section">
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px'}}>
                    ...
                    <button 
                        className={isAdding ? "btn-cancel" : "btn-primary"} 
                        onClick={() => setIsAdding(!isAdding)} 
                        style={{flex: 'none', width: 'fit-content', padding: '8px 20px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', borderRadius: '10px'}}
                    >
                        {isAdding ? 'CLOSE SEARCH' : <><Plus size={16} /> LOG FOOD</>}
                    </button>
                </div>
                ...
                {isAdding && (
                    <div className="food-search-box animate-fade-in" style={{marginBottom: '30px'}}>
                        ...
                        <div className="food-results grid">
                            {filteredFood.slice(0, 50).map((food, i) => (
